...
export const map = (array, func) => reduce(array, (result, value, index, array) => add(result, func(value, index, array)), []);
...
export function uniqueBy(array, func) {
  ...
  while(++i < array.length) {
    ...
    if(!find(resultKeys, (resultKey) => key === resultKey)) {
      result.push(array[i]);
      resultKeys.push(key);
    }
  }
...
export const qkSort = (array, func, ascending = true) => !ascending
  ? reverse(quickSort([...array], func))
  : quickSort([...array], func);
export const mgSort = (array, func, ascending = true) => !ascending
  ? reverse(mergeSort([...array], func))
  : mergeSort([...array], func);
...
export const qkHeuristicFind = (array, func) => qkSort(array, func, false)[0];
export const mgHeuristicFind = (array, func) => mgSort(array, func, false)[0];
export const heuristicFind = mgHeuristicFind;
...
export const insert = (array, index, ...values) => copy(array).splice(index, 0, ...values);
...
export function chain(initValue = []) {
  let value = initValue;
  const chain = {
    link: (func, ...args) => {
